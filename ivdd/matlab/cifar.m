...
for num=0:9
    for seed=0:0
        ...
        Xtg = load(strcat('./matlab_', dataset, '/x',int2str(num),'_seed',int2str(seed),'.txt'));
        Xval1 = load(strcat('./matlab_', dataset, '/xval1',int2str(num),'_seed',int2str(seed),'.txt'));
        Xval2 = load(strcat('./matlab_', dataset, '/xval2',int2str(num),'_seed',int2str(seed),'.txt'));
        ...
        %sigma = 0.18;
        %sigma = 1;
        gamma=10.0 / (var(Xtg, 0, "all") * size(Xtg,2))
        ...
        if (doCheck==1)
             ...
             %set(hLine2,'Marker','.');
             grid on;
             saveas(hh, strcat('./probs_', dataset, '/loss_',int2str(num),'_seed',int2str(seed),'.png'));

             close;
             close;
             %save('toy.mat');
             dlmwrite(strcat('./probs_', dataset, '/', int2str(num), '_seed', int2str(seed), '_alpha.txt'),alpha)
        ...
        fileID = fopen(strcat('./results_', dataset, '/',int2str(num),'_seed',int2str(seed),'.txt'),'w');
        ...
        dlmwrite(strcat('./probs_', dataset, '/', int2str(num), '_seed', int2str(seed), '_p_test.txt'),probability, 'precision','%.20f')
        
        dec_func_test = cat(1, f1, f2);
        dlmwrite(strcat('./probs_', dataset, '/', int2str(num), '_seed', int2str(seed), '_f_test.txt'),dec_func_test, 'precision','%.20f')

        f = figure;
        h2 = histogram(probability,10)
        set(h2,'facecolor','blue','facealpha',1)
        saveas(f, strcat('./probs_', dataset, '/test_',int2str(num),'_seed',int2str(seed),'.png'))
        close;
        
        dlmwrite(strcat('./probs_', dataset, '/', int2str(num), '_seed', int2str(seed), '_p_train.txt'),p_train, 'precision','%.20f')
        dlmwrite(strcat('./probs_', dataset, '/', int2str(num), '_seed', int2str(seed), '_f_train.txt'),f_train, 'precision','%.20f')

        f = figure;
        h2 = histogram(p_train,10)
        set(h2,'facecolor','blue', 'facealpha', 1)
        saveas(f, strcat('./probs_', dataset, '/train_',int2str(num),'_seed',int2str(seed),'.png'))
